...
export class EventBusService {
    ...
    connect(url: string, defaultHeaders: any = null, options: any = {}): void {
        if(this.state != EventBusService.STATE_CLOSED){
            return
        }
        // console.log(url)
        ...
        //     mess['type'] = 'edit';
        //     this.publish('auction.10',mess);
        //     // this.send('auction.10',{'message':'yosi'});
        // };
        ...
        this.sockJS.onopen = () => {
            ...
            // pingTimerID = setInterval(sendPrice, pingInterval*3);
            this.open.emit(null);
        };
    ...
    publish(address: string,
        ...
        if (this.connected) {
            ...
            this.addEventToQueue({address, body,headers,type: EventBusService.TYPE_PUBLISH});
        }
    };
    ...
    private addEventToQueue(event) {
        ...
        this.eventQueue = []
        this.eventQueue = JSON.parse(sessionStorage.getItem("eventQueue"))
        ...
        sessionStorage.setItem("eventQueue", JSON.stringify(this.eventQueue));
    }
        ...
        while (this.eventQueue.length > 0) {
            ...
            switch (event.type) {
                case EventBusService.TYPE_PUBLISH:
                    this.publish(event.address, event.body,event.action);
                    break;
